  ...
  const result = await pool.query(`
    WITH listed AS (
      SELECT c.id,c.process_id AS "processId",c.reference,c.status,c.description,c.contract_type AS "contractType",c.procurement_method AS "procurementMethod",c.signed_at AS "signedAt",c.signed_at,c.value::float8,c.added_days AS "addedDays",c.source_process_url AS "sourceUrl",
        ...
        (SELECT CASE WHEN COUNT(*)=0 THEN NULL ELSE json_build_object('status','analyzed','analyzerVersion',MAX(f.analyzer_version),'analyzedAt',MAX(f.analyzed_at),'categories',json_agg(json_build_object('category',f.category,'status',f.status,'documentId',f.document_id,'fileName',d.file_name,'sourceUrl',d.source_url,'pageNumber',f.page_number,'excerpt',f.excerpt,'matchedTerm',f.matched_term,'reviewDecision',r.decision,'reviewNote',r.note,'reviewedAt',r.reviewed_at) ORDER BY CASE f.category WHEN 'need' THEN 1 WHEN 'justification' THEN 2 WHEN 'budget' THEN 3 ELSE 4 END)) END FROM document_findings f LEFT JOIN documents d ON d.id=f.document_id LEFT JOIN document_reviews r ON r.contract_id=f.contract_id AND r.category=f.category WHERE f.contract_id=c.id) AS "documentExplanation",
