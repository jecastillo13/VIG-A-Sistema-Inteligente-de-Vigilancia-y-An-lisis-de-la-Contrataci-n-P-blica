...
type ContractRow = {
  ...
  rawValue: number; mode: string; risk: number | null; updated: string;
...
export default function Home() {
  ...
  const filtered = useMemo(() => contracts.filter((c) => {
    ...
    return matchesQuery && (!priorityOnly || (c.risk ?? 0) >= 70);
  }), [contracts, priorityOnly, query]);
      ...
      <section className="md:pl-60" id="panorama"><div className="mx-auto max-w-[1600px] p-4 md:p-7">
        ...
        <div className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1.6fr)_minmax(340px,.8fr)]">
          <section id="contratos" className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
            ...
            <div className="flex items-center justify-between border-t border-slate-200 px-5 py-3 text-xs text-slate-500"><span>Mostrando {filtered.length} de {contracts.length} contratos reales</span><span className="font-bold text-[#155b9b]">SECOP II · 2025</span></div>
          </section>
