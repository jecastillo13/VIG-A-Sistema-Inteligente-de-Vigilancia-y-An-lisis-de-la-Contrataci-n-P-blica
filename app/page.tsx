...
import {
  ...
  Menu, Network, Search, ShieldCheck, SlidersHorizontal, Users, X,
} from 'lucide-react';
...
export default function Home() {
  ...
  const filtered = useMemo(() => contracts.filter((c) => `${c.id} ${c.entity} ${c.supplier}`.toLowerCase().includes(query.toLowerCase())), [query]);
  ...
  return (
    <main className="min-h-screen bg-[#f4f6f8] text-slate-950">
      ...
      <aside className={`${navOpen ? 'flex' : 'hidden'} fixed bottom-0 left-0 top-16 z-20 w-60 flex-col border-r border-slate-200 bg-[#0d274b] p-3 text-blue-100 md:flex`}>
        <nav className="space-y-1 text-sm"><a className="flex items-center gap-3 rounded-xl bg-white/12 px-3 py-3 font-semibold text-white" href="#"><LayoutDashboard size={18}/>Panorama nacional</a><a className="flex items-center gap-3 rounded-xl px-3 py-3 hover:bg-white/8" href="#contratos"><FileSearch size={18}/>Contratos</a><a className="flex items-center gap-3 rounded-xl px-3 py-3 hover:bg-white/8" href="#"><Building2 size={18}/>Entidades</a><a className="flex items-center gap-3 rounded-xl px-3 py-3 hover:bg-white/8" href="#"><Users size={18}/>Proveedores</a><a className="flex items-center gap-3 rounded-xl px-3 py-3 hover:bg-white/8" href="#"><Network size={18}/>Red contractual</a></nav>
      ...
      <section className="md:pl-60"><div className="mx-auto max-w-[1600px] p-4 md:p-7">
        <div className="mb-6 flex flex-col justify-between gap-4 lg:flex-row lg:items-end"><div><p className="mb-1 text-xs font-bold uppercase tracking-[.16em] text-[#1e5b9d]">Monitoreo nacional · Vigencia 2026</p><h1 className="text-2xl font-bold tracking-tight md:text-3xl">Panorama de riesgo contractual</h1><p className="mt-1 text-sm text-slate-500">Actualizado hoy, 15:42 · Fuentes de demostración SECOP II</p></div><div className="flex gap-2"><button className="flex items-center gap-2 rounded-xl border border-slate-300 bg-white px-3.5 py-2.5 text-sm font-semibold"><SlidersHorizontal size={16}/>Configurar alertas</button><button className="rounded-xl bg-[#155b9b] px-4 py-2.5 text-sm font-semibold text-white shadow-sm">Generar informe</button></div></div>
        ...
        <div className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1.6fr)_minmax(340px,.8fr)]">
          <section id="contratos" className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
            <div className="flex flex-col gap-3 border-b border-slate-200 p-5 sm:flex-row sm:items-center"><div><h2 className="font-bold">Contratos de mayor riesgo</h2><p className="text-xs text-slate-500">Priorizados por indicadores objetivos y reproducibles</p></div><div className="ml-auto flex gap-2"><label className="flex min-w-0 items-center gap-2 rounded-xl border border-slate-200 px-3 py-2 text-sm focus-within:ring-2 focus-within:ring-blue-200"><Search size={16} className="text-slate-400"/><input value={query} onChange={(e)=>setQuery(e.target.value)} className="w-36 outline-none" placeholder="Buscar contrato" aria-label="Buscar contrato"/></label><button className="rounded-xl border border-slate-200 p-2.5" aria-label="Filtrar"><Filter size={17}/></button></div></div>
            <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead className="bg-slate-50 text-[11px] uppercase tracking-wider text-slate-500"><tr><th className="px-5 py-3 font-semibold">Contrato</th><th className="px-4 py-3 font-semibold">Entidad / proveedor</th><th className="px-4 py-3 font-semibold">Valor</th><th className="px-4 py-3 font-semibold">Modalidad</th><th className="px-4 py-3 text-center font-semibold">Riesgo</th></tr></thead><tbody>{filtered.map(c => <tr key={c.id} onClick={()=>{setSelected(c);setDetailOpen(true)}} className={`cursor-pointer border-t border-slate-100 transition hover:bg-blue-50/50 ${selected.id===c.id?'bg-blue-50/70':''}`}><td className="px-5 py-4"><strong className="text-[#174f8c]">{c.id}</strong><span className="mt-1 block text-xs text-slate-400">{c.updated}</span></td><td className="px-4 py-4"><strong className="block font-semibold">{c.entity}</strong><span className="text-xs text-slate-500">{c.supplier} · {c.department}</span></td><td className="px-4 py-4 font-semibold tabular-nums">{c.value}</td><td className="px-4 py-4 text-slate-600">{c.mode}</td><td className="px-4 py-4 text-center"><RiskBadge value={c.risk}/><span className={`ml-1 text-[10px] font-bold ${c.trend>0?'text-rose-500':'text-emerald-600'}`}>{c.trend>0?'+':''}{c.trend}</span></td></tr>)}</tbody></table></div>
